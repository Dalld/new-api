...
func (*CreemAdaptor) RequestPay(c *gin.Context, req *CreemPayRequest) {
	...
	if err := freezeCommissionSnapshot(topUp, user, selectedProduct.Quota); err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Creem 冻结返佣快照失败 user_id=%d trade_no=%s product_id=%s error=%q", id, referenceId, selectedProduct.ProductId, err.Error()))
